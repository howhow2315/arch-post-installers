...
# Returns 0 if the device is SSD, 1 if HDD (defaults to root filesystem)
_get_rotational_flag() {
    local target="${1:-/}" # Default to root if no argument given
    local dev blk rotational

    # Resolve the backing device for the mount point
    dev=$(findmnt -no SOURCE "$target") || return 1

    # If it's LVM/LUKS, get underlying physical device
    if [[ $dev =~ ^/dev/mapper/ ]]; then
        blk=$(lsblk -no PKNAME "$dev") || return 1
        blk="/dev/$blk"
    else
        blk=$(lsblk -no PKNAME "$dev") || return 1
        blk="/dev/$blk"
    ...
    rotational=$(cat /sys/block/$(basename "$blk")/queue/rotational 2>/dev/null) || return 1
